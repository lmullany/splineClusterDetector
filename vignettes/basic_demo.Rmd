...
Requirements for this scan statistic implementation are: - assignment of the observed data to geographic regions with centroid point locations for distance calculations to enable expanding cylinder scans - observed counts of the data in each region for each time step - corresponding expected counts of the data, estimated from baseline data - distances between the regions
...
-   using lower-level functions when stepwise control is needed,
...
The code below restricts the data to 12 locations for concise demonstration. Using the full dataset will not impact computation speed.
...
The functions zip_distance_matrix()`and`county_distance_matrix()\` provide full pairwise distance matrices, which may be sparse with many zeros, depending on the user data:
...
`us_distance_matrix()` is available for all U.S. counties, but is typically too large for routine examples:
...
This package also provides a function `st_detect()` to create artifical cluster signals for addition to background data in order to test and evaluate the detection capability of the `find_clusters`' method. A subsequent vignette will provide details on `st_detect()`. To create a clear demonstration signal, inject a small synthetic cluster first:
...
and demonstration data with tables of county and zip code locations and shape files:

-   `example_count_data`
-   `counties`
-   `zipcodes`

These can be used directly in workflows, examples, and tests. \## Example cluster
...
p_heat <- generate_heatmap(heatmap_data, plot_type = "ggplot")
class(p_heat)
plot(p_heat)
```
...
p_ts <- generate_time_series_plot(ts_data, plot_type = "ggplot")
class(p_ts)
plot(p_ts)
```
...
These are used by the `find_clusters()` function and may be useful for custom diagnostics and method development.
...
cat_tbl <- add_spline_threshold(oe_grid = oe, spline_lookup = "01")
```
