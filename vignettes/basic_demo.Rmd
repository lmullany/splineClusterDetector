...
The functions `tract_distance_matrix()`, `zip_distance_matrix()` and `county_distance_matrix()` provide full pairwise distance matrices, which may be sparse with many zeros, depending on the user data:
...
This package also provides a function `st_injects()` to create artificial cluster signals for addition to background data in order to test and evaluate the detection capability of the `find_clusters`' method. A subsequent vignette will provide details on `st_injects()`. To create a clear demonstration signal, inject a small synthetic cluster first:
