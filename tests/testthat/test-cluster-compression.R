...
testthat::test_that("compress_clusters smoke and structural invariants", {
  x <- .make_alert_table()
  cl <- gsClusterDetect::compress_clusters(
  ...
  dl <- gsClusterDetect::create_dist_list(
    level = "county",
    threshold = 50,
    st = "OH",
    unit = "miles"
  )
  cl_list <- gsClusterDetect::compress_clusters(
...
testthat::test_that("compress_clusters errors on wrong class and empty input", {
  ...
  testthat::expect_error(
    gsClusterDetect::compress_clusters(
...
testthat::test_that(
  ...
  {
    x <- .make_alert_table()
    cl_slow <- gsClusterDetect::compress_clusters(
      cluster_alert_table = data.table::copy(x[["alerts"]]),
      distance_matrix = x[["dm"]]
    )
    cl_fast <- gsClusterDetect::compress_clusters_fast(
      cluster_alert_table = data.table::copy(x[["alerts"]]),
      distance_matrix = x[["dm"]]
    )
    ...
    cl_fast_list <- gsClusterDetect::compress_clusters_fast(
      cluster_alert_table = data.table::copy(x[["alerts"]]),
      distance_matrix = dl
    )
...
testthat::test_that(
  "compress_clusters_fast validates class and empty alert table",
  {
    ...
    testthat::expect_error(
      gsClusterDetect::compress_clusters_fast(
        cluster_alert_table = data.table::data.table(),
        distance_matrix = x[["dm"]]
      ),
      "ClusterAlertTable"
...
testthat::test_that(
  ...
  {
    x <- .make_alert_table()
    cl <- gsClusterDetect::compress_clusters_fast(
      cluster_alert_table = data.table::copy(x[["alerts"]]),
      distance_matrix = x[["dm"]]
    )
    out <- gsClusterDetect::add_location_counts(cl, x[["cases"]])
    ...
    out0 <- try(
      gsClusterDetect::add_location_counts(cl, cases_df),
      silent = TRUE
    )
...
testthat::test_that("add_location_counts validates class requirements", {
  x <- .make_alert_table()
  testthat::expect_error(
    gsClusterDetect::add_location_counts(
      cluster_list = list(),
      cases = x[["cases"]]
    ),
    "class 'clusters'"
  ...
  testthat::expect_error(
    gsClusterDetect::add_location_counts(bad_cl, x[["cases"]]),
...
testthat::test_that("reduce_clusters_to_min filters by summed cluster count", {
  ...
  with_counts <- gsClusterDetect::add_location_counts(cl, x[["cases"]])

  unchanged <- gsClusterDetect::reduce_clusters_to_min(
    with_counts,
    minimum = 1
  )
...
testthat::test_that("reduce_clusters_to_min validates clusters input", {
  testthat::expect_error(
    gsClusterDetect::reduce_clusters_to_min(list(), minimum = 2),
    "class 'clusters'"
  )
})
