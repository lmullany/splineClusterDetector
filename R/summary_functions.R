...
#'   If the requested backend is unavailable, the function warns and falls back
#'   to the other backend when available.
...
generate_heatmap <- function(
  ...
  plot_type <- resolve_plot_backend(plot_type, "generate_heatmap()")

  if (plot_type == "ggplot") {
    ggplot_heatmap(heatmap_data)
  } else {
    plotly_heatmap(heatmap_data, ...)
  }
}
...
#' @param heatmap_data data frame generated by `generate_heatmap_data`
#' @param x name of date column, default is "date"
#' @param y name of location column, default is "y",
#' @param logscale boolean (default FALSE); set to TRUE to view on log scale
#' @keywords internal
...
#'   If the requested backend is unavailable, the function warns and falls back
#'   to the other backend when available.
#' @param locations string indicating location name (defaults to
#'   "All Locations")
#' @param ... passed onto plotly
#' @export
#' @returns a ggplot or plotly object
...
generate_time_series_plot <- function(
  ...
  plot_type <- match.arg(plot_type)
  plot_type <- resolve_plot_backend(plot_type, "generate_time_series_plot()")
...
#' @param time_series_data data frame generated by `generate_time_series_data`
#' @param locations vector of locations to limit to; default is "All Locations"
#' @keywords internal
...
resolve_plot_backend <- function(plot_type, fn_name) {
  has_ggplot2 <- requireNamespace("ggplot2", quietly = TRUE)
  has_plotly <- requireNamespace("plotly", quietly = TRUE)

  if (!has_ggplot2 && !has_plotly) {
    cli::cli_abort(
      paste0(
        "Neither {.pkg ggplot2} nor {.pkg plotly} is installed. ",
  ...
  if (plot_type == "ggplot" && !has_ggplot2) {
    cli::cli_warn(
      paste0(
        "Requested {.pkg ggplot2} backend, but {.pkg ggplot2} is not ",
        "installed. Falling back to {.pkg plotly}."
      )
    )
    return("plotly")
  }

  if (plot_type == "plotly" && !has_plotly) {
    cli::cli_warn(
      paste0(
        "Requested {.pkg plotly} backend, but {.pkg plotly} is not ",
        "installed. Falling back to {.pkg ggplot2}."
      )
    )
    return("ggplot")
  }
...
#' @param heatmap_data data frame generated by `generate_time_series_data`
#' @param locations vector of locations to limit to; default is "All Locations"
#' @keywords internal
